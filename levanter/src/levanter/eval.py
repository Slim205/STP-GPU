...
class TaggedEvaluator:
    ...
    def __init__(
        ...
        def accum_for_batch(
            ...
            with hax.axis_mapping(axis_mapping):
                ...
                mask = batch.loss_mask.astype(losses.dtype)  # [Batch, Token]
                this_tokens = hax.einsum("->", mask)
                this_loss = hax.einsum("->", mask, losses)  # to scalar
                ...
                safe_mean = hax.where(this_tokens, this_loss / this_tokens, 0.0)
                total_mean = total_mean.add(safe_mean, this_tokens)
                safe_mean_per_tag = hax.where(this_tokens_per_tag, this_loss_per_tag / this_tokens_per_tag, 0.0)
                mean_per_tag = mean_per_tag.add(safe_mean_per_tag, this_tokens_per_tag)
                
            return total_mean, mean_per_tag
    ...
    def evaluate(self, m: LmHeadModel):
        ...
        # logger.info(f'eval loss {total_loss.mean.item()}, {total_loss.total.item()}')
        # logger.info(f'eval loss per tag {mean_loss_per_tag_cpu} {total_tokens_per_tag_cpu}')
