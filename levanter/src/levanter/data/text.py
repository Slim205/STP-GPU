...
class LMDatasetSourceConfig:
    ...
    trust_remote_code: bool = True  # whether to trust remote code when loading hf datasets
    ...
    def get_shard_source(self, split) -> Optional[ShardedDataset[str]]:
        if self.id is not None:
            try:
                ds = WrappedHFDataset(self.id, split=split, name=self.name, streaming=self.stream, trust_remote_code=self.trust_remote_code)
